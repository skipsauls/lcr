...
var force = (function () {
    ...
    function refreshTokenWithPlugin(success, error) {
        ...
        oauthPlugin.authenticate(
            function (response) {
                ...
                tokenStore.forceOAuth = JSON.stringify(this.oauth);
                if (success) {
                    success();
                }
            },
     ...
     */
    function getUserId() {
        return (typeof(this.oauth) !== 'undefined') ? this.oauth.id.split('/').pop() : undefined;
    }
    ...
    function request(obj, successHandler, errorHandler) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                ...
                } else if (xhr.status === 401 && self.oauth.refresh_token) {
                    refreshToken(
                        function () {
        ...
        if (useProxy) {
            xhr.setRequestHeader("Target-URL", self.oauth.instance_url);
        }
        console.warn('sending: ', obj.data);
        window.thedata = obj.data;
